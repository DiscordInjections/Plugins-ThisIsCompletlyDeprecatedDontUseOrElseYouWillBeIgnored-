...
class CharacterCounter extends Plugin {
    ...
    init() {
        ...
        $(".chat .content textarea").off("input.charCounter").off("keydown.charCounterKey").on("input.charCounter", eventhandler).on("keydown.charCounterKey", eventhandler);
    }

    unload() {
        this.mo.disconnect();
        if(document.querySelector("#charcounter")) $("#charcounter").remove();
        $(".chat .content textarea").off("input.charCounter").off("keydown.charCounterKey");
    }
}
