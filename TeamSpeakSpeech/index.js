...
class TeamSpeakSpeech extends Plugin {
    ...
    load() {
        this.settings = $.extend({}, {enabled:true,volume:1,rate:1,pitch:1}, JSON.parse(window.DI.localStorage.getItem("TeamSpeakSpeech")));
        this.save();
    }

    save() {
        window.DI.localStorage.setItem("TeamSpeakSpeech", JSON.stringify(this.settings));
    }

    VC() {
        window.DI.client.channels.get(this.currentVC);
    }
